...
import { WeChatCredentials, WeChatAccount } from '../../../types';
...
export const useWeChatManager = (onError: (msg: string) => void): UseWeChatManagerReturn => {
  ...
  const handleAddWeChatAccount = () => {
    ...
    // If adding default account, update current account
  ...
  const handleDeleteWeChatAccount = (accountId: string) => {
    if (window.confirm('确定要删除这个微信账号吗？')) {
  ...
  const handleTestWeChatAccount = async (accountId: string) => {
    ...
    try {
      ...
      if (isValid) {
        // If test successful, automatically set as current account
