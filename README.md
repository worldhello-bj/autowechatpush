...
创新的双AI模式，通过两次后端调用实现专业化分工：

- **第一次调用 - 文案AI (Content AI)**: 专注于内容创作、故事叙述、关键词提取
- **第二次调用 - 美化AI (Design AI)**: 专注于排版设计、颜色搭配、视觉呈现
- **优势**: 分两次调用节省上下文，使每个AI更加专精于各自领域
...
- 图片上传到微信服务器
- 一键发布到公众号草稿箱
- **🆕 微信文章导入**：一键导入微信公众号文章，自动提取排版结构

### 🔗 微信文章一键导入 🆕

**"学其形,弃其神"** - 快速学习优秀文章的排版结构

- **智能拆解**：输入微信文章链接，系统自动提取文章结构
- **版权保护**：强制替换所有图片为 SVG 占位符（data URL），仅保留排版骨架，不存储任何原图
- **SVG识别**：自动识别并提取SVG装饰元素
- **结构还原**：在编辑器中完美还原原文排版布局
- **免责声明**：首次使用显示版权提示，仅供学习使用
- **服务端处理**：所有图片替换在后端完成，客户端不存储任何外部图片
...
- 所有外部图片将被替换为 SVG 占位符（通过 data URL 嵌入，不在客户端存储）
- 本功能仅供学习排版思路，请勿用于商业用途
- 请尊重原作者版权，仅学习排版结构
- 图片替换完全在后端服务器处理，确保版权合规
...
- 注册的用户自动保存到 `backend/data/users.json`
- 包括用户信息、角色、配额等完整数据
- 服务器重启后自动恢复，数据不丢失
- 管理员在后台创建、修改、删除用户的操作都会持久化

### 📊 用户数据分析 🆕

- 自动追踪用户行为（登录、文章生成、发布、保存等）
- 管理员后台数据分析面板，查看用户活跃度和使用统计
- 支持12种事件类型，包括AI使用、设置修改等
- 数据持久化到 `backend/data/analytics.json`
- 详细使用说明见 [ANALYTICS.md](./ANALYTICS.md)
...
双AI模式通过两次后端调用实现专业化分工：

1. 选择 DeepSeek 或 Qwen 模型
2. 开启 **双AI模式** 开关
3. 输入主题后点击 **双AI生成**
4. 系统自动执行：
   - **第一次调用**：内容AI专注于创作高质量文章内容
   - **第二次调用**：设计AI专注于优化排版和视觉呈现
5. 优势：分两次调用节省上下文，使每个AI更加专精
